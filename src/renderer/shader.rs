...
fn compile_program<F: glium::backend::Facade>(display: &F, vs: &String, fs: &String) 
...
fn print_err(err:  glium::program::ProgramCreationError){
    match err{
        glium::program::ProgramCreationError::CompilationError(x) => print!("{}\n", x),
        glium::program::ProgramCreationError::LinkingError(y) => print!("{}\n", y),
        z => println!("{:?}", z),
    }
}
