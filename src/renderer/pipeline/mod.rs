...
where T: Ord + Copy + 'a
{
    fn new (pipe: &'a Pipeline<CTX,T>) -> Runner<'a,CTX,T>{
        Runner{
            available: BTreeSet::new(),
	 		ready: pipe.get_stages().iter().filter(|x| x.get_inputs().is_empty() ).collect::<VecDeque<&BStage<CTX,T>>>(),
	 		wait: pipe.get_stages().iter().filter(|x| !x.get_inputs().is_empty() ).collect::<Vec<&BStage<CTX,T>>>(),
        }
    }
	...
	fn commit(&mut self, outputs: &[T]){
		...
		let new_ready = self.wait.iter().filter(|s|{
									s.get_inputs().iter()
										.map(|i| self.available.contains(i) )
										.fold(true, |acu, c|  acu && c )
								}).map(|x| *x).collect::<Vec<&BStage<CTX,T>>>();
		self.ready.extend(new_ready.as_slice());
		self.wait = self.wait.iter().filter(|s|{
									!s.get_inputs().iter()
										.map(|i| self.available.contains(i) )
										.fold(true, |acu, c|  acu && c )
								}).map(|x| *x).collect::<Vec<&BStage<CTX,T>>>();
...
mod tests {
    ...
    fn stage() {
        ...
        let _ = StageInstance::new(inputs.as_slice(),
			&[], 
	    	 |ctx: &mut i32, inputs| {
	    		println!("{} {:?}", ctx, inputs);
	    	}
